...
\begin{frame}{Session objective}\begin{itemize}\item Combine economic prior information with the likelihood.\item Understand posterior simulation and convergence diagnostics.\item Produce a reproducible Bayesian DSGE analysis in Dynare.\end{itemize}\vfill\key{Handout:} Chapter 4 \quad \key{Code:} \texttt{chap4/codes/}\end{frame}
...
\begin{frame}{From posterior mode to simulation}\centering\begin{tikzpicture}[node distance=.72cm,>=Latex,every node/.style={draw,rounded corners,align=center,minimum width=2.2cm,minimum height=.7cm,fill=gray!8}]
...
\begin{frame}{From theory to credible inference}\centering\begin{tikzpicture}[node distance=.8cm,>=Latex,every node/.style={draw,rounded corners,align=center,minimum width=2.25cm,minimum height=.7cm}]
