...
\begin{itemize}
\item Solving a rational expectation model;

\item Linearizing a system of equations;

\item Extending a model and checking its solution.
\end{itemize}
...
\textbf{Economic intuition.} Capital links current investment to future
production. \textbf{Bayesian/econometric intuition.} The equilibrium solution
becomes the transition equation used by the Kalman filter. \textbf{Model.}
Derive the steady state, log-linearize, and verify determinacy.
\textbf{MATLAB/Dynare exercise.} Compare analytical and Dynare solutions, then
interpret moments and IRFs. \textbf{Takeaway.} Never estimate a DSGE model
before residual and Blanchard--Kahn checks pass.
...
\section{Solving a linear rational expectation model}
...
in the parameter block below. The syntax for assigning parameter values is
