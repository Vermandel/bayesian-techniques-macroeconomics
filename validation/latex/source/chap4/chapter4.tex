...
\begin{itemize}
\item Estimating a DSGE model using Bayesian techniques;

\item Decomposing business cycles into multiple sources of exogenous disturbances;

\item Producing and evaluating forecasts.
\end{itemize}
...
\textbf{Economic intuition.} Priors encode disciplined information about
structural mechanisms; the likelihood confronts them with data.
\textbf{Bayesian intuition.} The posterior combines both sources and MCMC
quantifies uncertainty beyond the mode. \textbf{Model and data.} Check support,
measurement equations, identification, and independent shocks.
\textbf{MATLAB/Dynare exercise.} Estimate, inspect acceptance and mixing, and
report posterior IRFs with credible intervals. \textbf{Takeaway.} Posterior
claims require reproducible convergence and sensitivity diagnostics.
...
strategy is presented by \cite{smets2003estimated} as follows:
...
possible to get of solution of the form
\begin{equation}
\hat{x}_{t}=F\hat{x}_{t-1}+G\epsilon_{t}, \label{eq: non linear DR}%
\end{equation}
where elements in matrices $F$\ and $G$ are function of structural parameters
$\Theta$.
...
The Bayesian approach consists in defining an \textit{apriori} distribution
...
Bayes' rule:
\begin{equation}
p(\theta|Y)=\frac{p(\theta|Y)p(\theta)}{p(Y)}\approx L(\theta|Y)p(\theta),
\end{equation}
where $L(\theta|Y)$ denote the likelihood function, $p(\theta|Y)$ is the
posterior distribution of parameters and $p(Y)$ is the marginal likelihood
...
be constructed numerically by applying the Markov Chain Monte Carlo (MCMC)
methods. Finally, we compute the posterior moments of the parameters using a
sufficiently large number of draws, having made sure afterwards the MCMC
...
\resizebox{\linewidth}{!}{\texttt{estimation(datafile=mydata, first\_obs=1,
mh\_replic=0, mh\_jscale=0.5, prefilter=1, lik\_init=2, load\_mh\_file,
mode\_file=NK3eq\_estim\_mode, mode\_compute=0)}}
